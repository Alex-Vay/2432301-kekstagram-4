...
function closeByBodyClick(evt) { //function должна использоваться как бы совместно с hideMessage, иначе они закольцуются, то есть нужно (всплытие)
  if (!(evt.target.closest('.success__inner') || evt.target.closest('.error__inner'))) {
    hideMessage();
  }
}

function closeByEscape(evt) { //function должна использоваться как бы совместно с hideMessage, иначе они закольцуются, то есть нужно (всплытие)
  if (isEscapeKey(evt)) {
    evt.preventDefault();
    hideMessage();
  }
